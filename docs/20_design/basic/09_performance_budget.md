...
- 版: 0.1（起草 claude、2026-08-30）。上位正本: 要件定義書 v0.2 REQ-NFR-01/02/03、REQ-UI-04a/b、OPEN-03
- OPEN-03 の決定（オーナー 2026-08-30）: **「スマホ全部」**。本章はこれを「最低対象端末を広い床で固定し、予算は床から導出する」と解釈する（§1）。床の機種スペックは Claude の提案値であり、オーナーが変更すれば数値だけ再導出する

## 1. 端末の定義（OPEN-03 クローズ）

| 名称 | 定義 | 代表スペック | 用途 |
|---|---|---|---|
| **最低対象端末（床）** | 2018 年以降の実売エントリー機。Android 8.0 (API 26) 以上、iOS 15 以上 | CPU: Snapdragon 450 / Helio P22 相当（Cortex-A53 ×8 @1.8 GHz）、RAM 2 GB（アプリ利用可能 ≤ 512 MB）、GPU なし前提（Skia/Impeller の CPU パス） | REQ-UI-04b、REQ-NFR-02 の下限。**全予算の導出元** |
| **基準端末（ミドル）** | 2021 年以降のミドルレンジ | Snapdragon 6xx / Tensor 下位相当（Cortex-A76 級 ×2 + A55 ×6）、RAM 4 GB | REQ-UI-04a、REQ-NFR-02 の p95 計測、テスター 1 台以上（REQ-USER-02） |
| **基準 PC** | GitHub Actions `ubuntu-latest` ランナー（4 vCPU）。CI しきい値の正本 | 変動を吸収するため予算は 3 回中央値。オーナー PC は参考値 | REQ-NFR-01/03 |

「全スマホ」の外側（API 25 以下、RAM 1 GB 台、Android Go 1 GB）は非対象と明記し、ストア/配布ページに最低要件として記載する（REQ-NFR-08 の対外文リント対象）。

## 2. 予算（確定値。CI・実機計測のしきい値）

| ID | 項目 | 床（最低対象） | 基準端末 | 基準 PC | 導出 |
|---|---|---|---|---|---|
| PB-01 | 1 tick（64×64、4 系統、全 7 phase）p95 | ≤ 8 ms | ≤ 4 ms | ≤ 0.5 ms | REQ-NFR-02 の 4 ms を基準端末に置き、床は単コア性能比 ≈ 1/2 で 2 倍。PC は 2,000 tick ≤ 1.0 s から |
...
| PB-04 | フレーム p95（描画 + tick 実行を含む UI スレッド） | ≤ 33.3 ms（30 fps） | ≤ 16.7 ms | — | 床は 30 fps を許容。tick は UI スレッド外（isolate）で実行し、フレームは snapshot コピーのみ |
| PB-05 | 拡散のみ 2,000 tick | — | — | ≤ 200 ms | REQ-NFR-01（D2 で criterion） |
| PB-06 | headless 1 seed 2,000 tick | ≤ 16 s | ≤ 8 s | ≤ 1.0 s | REQ-NFR-01 |
| PB-07 | 常駐メモリ（アプリ全体、Flutter 込み） | ≤ 96 MB | ≤ 96 MB | — | 床のアプリ利用可能 512 MB に対し余裕。うち **SimCore ≤ 32 MB**（REQ-NFR-02、台帳は region 集約が前提 = BD-01 r4 §5） |
| PB-08 | 保存サイズ（SaveEnvelope） | ≤ 5 MB | 同左 | — | REQ-NFR-02、BD-10 |
| PB-09 | 起動〜開始画面 | ≤ 4 s | ≤ 2 s | — | 導出（テスター離脱防止） |
...
| D7 | PB-10 | バッチジョブ実測 |
| D12 | PB-01..04, PB-07..09（実機） | 実機 MEAS。床 1 台 + 基準端末 1 台で計測し docs/50_records/ に記録。床が無ければ Android エミュレータ（cores=2, RAM 2 GB, x86 → 参考値扱い、合否には使わない） |
