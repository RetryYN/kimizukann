...
- 版: 0.3（起草 claude、2026-08-30。r2: 床を 2021 年以降に更新。r3: grok 審査 7 major 反映。r4: 床機種列から SD680/G85 を除外（A76 級ではない） — スレッド模型固定、PB-07 を REQ どおり 32 MB、一巡 10 分、iOS 仮置き、トレース修正）。上位正本: 要件定義書 v0.2 REQ-NFR-01/02/03、REQ-UI-04a/b、OPEN-03
...
| 名称 | 定義 | 代表スペック | 用途 |
|---|---|---|---|
| **最低対象端末（床）** | **2021 年以降**の実売エントリー機。Android 11 (API 30) 以上。**検証版は Android のみ**（REQ-OPS-07 / OPEN-07）。iOS 15 以上（iPhone 13 / SE3 以降）は**仮置き**で、実機試験まで配布文に書かない | CPU: Snapdragon 480 / Dimensity 700 相当（Cortex-A76 級 ×2 + A55 ×6）。**SD680 (A73×4+A53×4)・Helio G85 (A75×2+A55×6) は A76 級でないため床に含めない**（含めるなら PB-01/06 を再計算する）、RAM 3 GB（アプリ利用可能 ≤ 1 GB）、GPU あり（Impeller） | REQ-UI-04b、REQ-NFR-02 の下限。**全予算の導出元** |
