...
| **LedgerSave** | フロー台帳（region 単位）、スタンプ列、z 窓（10 tick 平均×20） | SimCore（台帳）／Explain（窓・スタンプ） | ✕（hash 外だが **台帳ダイジェスト** = SHA-256(集約後 LedgerRecord を tick, region_id, lineage, reason, from, to, amount の順に LE 直列化（発生時 `LedgerEntry` は cell_index 粒度。ダイジェスト/LedgerSave は tick 終了時に region へ集約した `LedgerRecord { tick: u32, region_id: u8 (0..=15, スタンプの region_ids と同幅), lineage: u8, reason, from_pool, to_pool, amount: 和 }` を tick→region_id→lineage→reason→from→to の順にソートしたもの） → スタンプを tick, kind, region_ids の順 → z 窓を pool, 系統, 値の順) を三経路 AT で比較） | WorldSave と同じトランザクションで保存。**再計算しない**（復帰後のイベント列一致のため） |
