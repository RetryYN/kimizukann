...
- 版: 0.3（起草 cursor-kimi、2026-08-30。PR #1 レビュー r1 反映: mutation を初期配置ばらつき専用・interaction を初期配置位置抽選に割当、「予備」禁止を明記。0.3: D4-Q3 裁定（2026-08-30 Claude 採用）により create 行のアレル抽選を「配置セル × 系統ごと」から「系統ごと 1 回」へ改訂。LineageParams は系統集約（BD-03）でセル別特性を保持できず、REQ-GEN-01 と整合させるため）
...
| mutation | 初期配置の離散アレル分布（ばらつきレバー）専用。検証版で変異の計算侵入はなく、tick 中の消費は常に 0 | create 時の初期配置のみ | 確定（系統ごと 1 語。DD-D4 §4 で詳細化） | REQ-GEN-08, REQ-OUT-01 |
...
| create（初期配置） | 0 | 0 | 系統ごとに 1 回（アレル抽選。計 系統数 × 1 語） | 配置セル × 系統ごとに 1 回（位置抽選） | ばらつきレバー・ランダム配置（D4 で確定）。tick 進行中は全ストリーム 0 |
