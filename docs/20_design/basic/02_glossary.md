...
## 10. 説明器文言規約・文体・禁止語リスト（REQ-EXP-02/06, REQ-NFR-07/08, REQ-UI-02）

### 10.1 説明器の4段文体規約（REQ-EXP-01, REQ-EXP-02, REQ-EXP-05, REQ-NFR-07）

「今どうなってる？」および終了時説明は、以下の **4段固定構造・ラベル区別・文体規則** に従う。順序変更や段の省略は禁止（REQ-EXP-01）。

| 段 | 区分ラベル | 記述対象 | 文体規則・語尾規約 | 記述例（通常時 / 絶滅時） |
|---|---|---|---|---|
| 1 | `[事実]`（観測データ） | フロー台帳・スナップショット・スタンプの実測値 | 観測事実のみを客観記述（推論・解釈は含めない）。「〜が発生」「〜を確認」「〜は X mass_u であった」 | 「tick 120 に中央領域でアオシキの急増スタンプを記録。栄養プールが 30% 減少した。」 / 「tick 450 にアカバエの総生体量が 0 となり絶滅判定となった。」 |
| 2 | `[解釈]`（モデル推論） | 理由コード（RESOURCE_SKEW / REPRO_DRIVEN / NICHE_MISMATCH / LOW_DISPERSION / NONE）に基づく要因分析 | モデルによる有力推測として記述（断定形「〜である」は禁止）。「〜と考えられる」「〜が主因と推測される」「〜の可能性が高い」 | 「繁殖エネルギーの余剰が急増の主因と考えられる（REPRO_DRIVEN）。」 / 「高維持コストに対し局所的な栄養補給が追いつかず、飢餓死骸化が連鎖したと推測される（RESOURCE_SKEW）。」 |
| 3 | `[未知]`（データ不足） | 単一 seed 観測の限界・未確定要因・他領域/他系統への波及影響 | 未確定・未検証であることを明示。「〜は現時点では不明」「別の初期条件でも同様かは未検証」「〜の影響は判別できない」 | 「この seed 単体では初期位置の有利さによるものかは断定できない。」 / 「適応方針（札）の不適合か、初期配置の近接競合が主因かは未検証。」 |
| 4 | `[次の一手]`（操作推薦） | もしもレバー（配置／個体数／適応方針／ばらつき）へ繋ぐ具体的な1つの再実験提案 | 1つのレバー変更に絞った提案調。「次は【レバー名】を〜してみてはどうか」「〜を変更して再実験しよう」 | 「次は【配置】でシロナミを中央近くに配置して変化を観察してみてはどうか。」 / 「次は【適応方針】で維持コストの低い札を選び、飢餓耐性を試してみよう。」 |

- **絶滅時の対応（REQ-EXP-05）**: 全滅・絶滅時にも上記の 4 段を維持し、原因候補（理由コード）と再実験の入口（もしもレバー）を必ず提示する。
- **区別の徹底（REQ-EXP-02）**: 事実（台帳データ）、解釈（理由コードによるモデル推論）、未知（未確定事項）の 3 者を表示上で明確に区別し、混同させない。

### 10.2 禁止語リスト（文言リント対象、REQ-EXP-06, REQ-NFR-07, REQ-NFR-08, REQ-UI-02）

文言リント（CI / INSP）により、説明器テンプレート、UI 表示文、対外文書における以下の語句・表現を機械的に検出・禁止する。

**否定除外規則（確定）**: 禁止語のマッチは「**否定語尾を伴わない出現**」に限定する。機械定義: 禁止語の直後に否定（`わけでは` / `とは限` / `ことはない`）が続く場合は非該当とする否定先読みを各パターンに付ける。例: カテゴリ 1 の `毎回こうなる` は `毎回こうなる(?!わけでは|とは限|ことはない)` とする。これにより REQ-UI-02 の固定 2 文「…毎回こうなるわけではありません。」は**非該当**、「毎回こうなる。」（断定）は**該当**となる。他カテゴリのパターンにも同じ方式の否定除外を必要に応じて付ける。

| カテゴリ | 禁止語・表現（Regex / 完全一致） | 禁止理由 | 推奨される代替表現 |
|---|---|---|---|
| 単一結果の一般化・断定（REQ-EXP-06, REQ-NFR-07） | `毎回こうなる(?!わけでは|とは限|ことはない)` / `必ず.*になる` / `絶対に` / `これが最適解` / `最強の` / `これが真理` / `証明された` / `必然的` | 単一 seed / 特定環境の偶然を普遍法則と誤認させることを防止 | 「今回の seed では〜となった」「この条件下では〜の傾向が見られる」「〜という結果が得られた」 |
| 疑似科学・擬人化・目的論（REQ-NFR-07） | `意志を持って` / `〜しようと進化した` / `目的を持って変異` / `自ら適応した` / `生命の誕生` / `本物の進化` | 個体群動態・数理モデルと生物進化の混同防止、目的論的説明の排除 | 「環境淘汰により〜が残存した」「〜の形質が有利に働いた」「パラメータの作用により〜」 |
| 誇大広告・対外禁止表現（REQ-NFR-08） | `本物の進化` / `最強を残そう` / `放置で育つ` / `宇宙.*ロードマップ` / `動物.*ロードマップ` | ゲーム性質の誤認防止、検証版スコープ逸脱の排除 | 「生態シミュレーション」「適応方針の比較検証」「観察と再実験」 |
| UI・ガイダンス内部用語（REQ-UI-02） | `別 seed` / `固定された偶然` | ユーザーの直感的理解を妨げる内部実装用語の口頭・画面露出防止 | 「別の初期条件」「異なる偶然のパターン」 |

### 10.3 チュートリアル固定文言（REQ-UI-02）

初回チュートリアル（固定 seed）開始時の画面表示文言は、REQ-UI-02 の原文どおり以下の **2 文のみ** に限定し、余計な説明文を追加してはならない。§10.2 の否定除外規則により本 2 文は禁止語リント非該当である（例外ではなく規則の適用結果）。
...
- **文言規約**: 説明器 4 段文体規約（事実/解釈/未知/次の一手）、禁止語リスト 4 カテゴリ（一般化断定/疑似科学/誇大対外/内部用語）、チュートリアル固定文言（2文）
