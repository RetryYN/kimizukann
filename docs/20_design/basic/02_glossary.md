...
|---|---|---|---|
| 単一結果の一般化・断定（REQ-EXP-06, REQ-NFR-07） | `毎回こうなる` / `必ず.*になる` / `絶対に` / `これが最適解` / `最強の` / `これが真理` / `証明された` / `必然的` | 単一 seed / 特定環境の偶然を普遍法則と誤認させることを防止 | 「今回の seed では〜となった」「この条件下では〜の傾向が見られる」「〜という結果が得られた」 |
...
初回チュートリアル（固定 seed）開始時の画面表示文言は、以下の **2 文のみ** に限定し、余計な説明文を追加してはならない。禁止語リスト（§10.2）に抵触しない文言とする（リント例外は設けない）。
> 「変化が見やすい初期条件です。結果は条件と乱数で変わります。」
