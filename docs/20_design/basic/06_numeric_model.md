...
- 入力: `docs/10_requirements/要件定義書_検証版_v0.2.md`（sign-off 済）、BD-05（契約）§5
...
| N7 | energy[L] | 10^6（= 1.0）。intake の energy 加算は 1.0 で飽和し、溢出分は熱散逸としてエネルギー台帳に記録（物質を減らさない。BD-05 §3） | 確定 | phase 適用後の値域検査 | REQ-SIM-08 |
...
| P11 | 終了判定 | Σbiomass（全セル・全系統） | 保存則により Σ ≤ M_max = 2×10^14（4,096×8×M_max の配置は保存則上到達不能） | 2×10^14 | 48 | i64 | i64 に収まる。集計は防御的に i128 で行い結果を検査 |
| P12 | 終了判定 | ε = 初期総生体量 × 100 / scale（1e-4 の Fixed 表現は 100） | 2×10^14 × 100 | 2×10^16 | 55 | i128 | 除算後 ≤ 2×10^10 |
