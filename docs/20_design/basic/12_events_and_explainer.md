...
- 版: 0.3（起草 cursor-kimi、2026-08-30。r1: grok 審査反映 — |z|_clip3 の正規化除去（検出不能の解消）、系統絶滅と世界 Extinct の分離、途中逆転の機械定義、理由コードの統計量を本章で定義。r2: grok nit 反映 — 逆転は窓始点/終点の比較と明記、REPRO_DRIVEN の分母は正の増分のみ。文言担当表記を gemini → kimi（BD-02 §10）に更新）
...
- 数値は「確定 / 初期仮説（D8 の較正で確定、OPEN-04）」を明記する。文言本文（禁止語リスト・4 段の文体）の正本は BD-02 §10（用語・文言担当: cursor-kimi）とし、本章は構造のみ定義する
...
| 逆転 | **培養中のシェア入れ替わり**: 窓（20 サンプル）の**始点と終点で**総生体量 1 位を比較し、終点の 1 位が tick 0 順位（BD-05 §1 の tick0_ranking）3 位以下の系統であれば検出。終了時ラベル Reversal（REQ-END-04b）は終了時のみの判定であり別物（途中逆転を拾うのが本種別。REQ-GOAL-03） |
...
- 理由コード（top_contributors の候補）と統計量（台帳・観測値からの機械定義。閾値 θ_* は初期仮説、D8 較正で確定、OPEN-04）:
  ...
  - `REPRO_DRIVEN`（繁殖由来）: 対象系統の直近窓の biomass 増分に占める Reproduction エントリ由来量の割合 = ΣReproduction amount ÷ Σ（biomass を**増やす**理由のエントリ量。分母は正の増分のみで減少分は含めない）が θ_repro 超
...
- 単一 seed からの断定表現を禁止する禁止語リスト（「毎回こうなる」等）は文言リントで機械検査。リスト本文の正本は BD-02 §10.2（用語・文言担当: cursor-kimi）。確定。参照: REQ-EXP-06
