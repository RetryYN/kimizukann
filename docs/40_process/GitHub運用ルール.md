...
- code PR の `evidence` は、その head に対する CI の `verify`（ubuntu / windows）両 job で一致した `state_hash` を用いる。審査者のローカル実行は任意（推奨）であり必須ではない。署名票の `checklist` には `evidence=CI` と明記し、CI 両 job の一致を確認した記録をレビューコメントに残す。
