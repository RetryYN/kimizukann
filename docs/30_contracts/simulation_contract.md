...
係数は入力質量に対する出力質量の割合で、合計1.0（固定小数点では1_000_000）とする。変換の余りは常に主出力プールへ戻し、拡散の余りは送り元セルに残す。余りは捨てない。
...
コア状態と係数はi64固定小数点、scale=1_000_000（10進6桁）とする。乗算の中間値はi128へ拡張する。除算はゼロ方向へ丸める。変換余りは主出力プールへ、拡散余りは送り元セルへ戻す。負値、i64範囲外、中間i128範囲外はエラーとする。
