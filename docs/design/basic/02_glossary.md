...
- 入力: `docs/要件定義書_検証版_v0.2.md`、`docs/contracts/simulation_contract.md`、`crates/sim-types/src/lib.rs`、`docs/相談会/第2回_統合案_v0.5.md`
...
## 9. 未定義語・検証版対象外機能（要 RFC）

※以下の用語は製品企画・相談会で言及されたが、検証版（v0.2）では意図的にスコープ外（REQ-OUT-01〜03）とされたか、初期ストア版以降の仕様策定待ち（要 RFC）となっている概念である。

| 語（日本語） | 識別子（英語） | 定義 | 型 | 単位・値域 | 出典 REQ・状態 |
|---|---|---|---|---|---|
| 培養カプセル | `culture_capsule` | 未定義（要 RFC）。初期条件・モデル版・seedをQR/コード化して端末間で培養を再生する共有形式 | 未定義（要 RFC） | 未定義（要 RFC） | REQ-OUT-03, 統合案 v0.5 2.4（初期版候補） |
| 一セル顕微鏡 | `cell_microscope` | 未定義（要 RFC）。特定セルの物質受け渡しアニメーションを局所リプレイ再生するUI機能 | 未定義（要 RFC） | 未定義（要 RFC） | REQ-OUT-03, 統合案 v0.5 2.2（初期版候補） |
| 観察の約束 | `observation_promise` | 未定義（要 RFC）。開始時に指定した観察対象領域が転換点に近づいた際に減速・強調する機能 | 未定義（要 RFC） | 未定義（要 RFC） | REQ-OUT-03, 統合案 v0.5 2.2（初期版候補） |
| 観察者の付箋 | `observer_note` | 未定義（要 RFC）。生命史カードにプレイヤー自身の感想や仮説メモを添付する機能 | 未定義（要 RFC） | 未定義（要 RFC） | REQ-OUT-03, 統合案 v0.5 2.4（初期版候補） |
| 三秒の転換標本 | `three_second_specimen` | 未定義（要 RFC）。最大転換点の前後3秒間をループ再生して閲覧する標本カード機能 | 未定義（要 RFC） | 未定義（要 RFC） | REQ-OUT-03, 統合案 v0.5 2.4（初期版候補） |
| 名付け | `naming` | 未定義（要 RFC）。新系統が発生した際にプレイヤーが固有名称を命名する機能 | 未定義（要 RFC） | 未定義（要 RFC） | REQ-OUT-01, 統合案 v0.5 2.3（初期版候補） |
| コドン編集・遺伝子編集 | `codon_editing` | 未定義（要 RFC）。系統の遺伝子配列やコドンを詳細に手動編集する機能 | 未定義（要 RFC） | 未定義（要 RFC） | REQ-OUT-01（検証版対象外） |
| 捕食 | `predation` | 未定義（要 RFC）。ある系統が生きた別系統の生体量を直接摂取・捕食する相互作用 | 未定義（要 RFC） | 未定義（要 RFC） | REQ-OUT-01, REQ-SCOPE-05（検証版は死骸利用のみ） |
| 分岐（種分化） | `speciation` | 未定義（要 RFC）。シミュレーション進行中に既存系統から新たな独立系統が発生・分岐する現象 | 未定義（要 RFC） | 未定義（要 RFC） | REQ-OUT-01（検証版対象外） |
| 突然変異（計算侵入） | `mutation_active` | 未定義（要 RFC）。計算ループ内で系統形質値が動的に変異・改変される機構 | 未定義（要 RFC） | 未定義（要 RFC） | REQ-OUT-01（検証版対象外） |
| 裏返し培養 | `reverse_culture` | 未定義（要 RFC）。系統名や形質札を伏せた状態で結果から推理するゲームモード | 未定義（要 RFC） | 未定義（要 RFC） | REQ-OUT-03, 統合案 v0.5 2.5（後続候補） |
| 生命史の音 | `life_history_sound` | 未定義（要 RFC）。生態系の状態変化や転換点を音響効果でフィードバックする機能 | 未定義（要 RFC） | 未定義（要 RFC） | REQ-OUT-03, 統合案 v0.5 2.5（後続候補） |
