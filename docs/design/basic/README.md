...
入力: `docs/要件定義書_検証版_v0.2.md`（sign-off 済み）。各章は担当が起草し、章ごとに「検証方法」列を持つ。**AT 設計（BD-08）が書かれ、trace で全 P0 要求が章と AT に紐付くまで基本設計は未完成**。
...
| BD-05 | `05_contract.md` | 既存 `docs/contracts/simulation_contract.md` を本章として再編。公開 API（Rust シグネチャ）、7 操作 FFI（C ABI・バッファ規約）、schema、事前/事後条件、semver 規則 | kimi | Claude | `cargo public-api` の出力と一致。各 pub 項目に REQ 参照 |
