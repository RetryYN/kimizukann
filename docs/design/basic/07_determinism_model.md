...
- 入力: `docs/要件定義書_検証版_v0.2.md`（sign-off 済）、BD-05（契約）§6・§10
...
| mutation | 初期配置の離散アレル分布（ばらつきレバー） | create 時の初期配置のみ（run 中は消費しない） | 初期仮説（D4 で確定） | REQ-GEN-08 |
| interaction | 予約（検証版では使用しない。捕食・分岐は対象外） | なし | 確定 | REQ-OUT-01, REQ-SCOPE-05 |
...
| create（初期配置） | 0 | 0 | 配置セル × 系統ごとに 1 回 | 0 | ばらつきレバーのアレル抽選（D4 で確定）。tick 進行中は 0 |
