...
srs = sorted(glob.glob(os.path.join(ROOT, 'docs', '要件定義書_検証版_v*.md')))[-1]
...
design_files = glob.glob(os.path.join(ROOT, 'docs', 'design', '**', '*.md'), recursive=True)
...
io.open(os.path.join(ROOT, 'docs', 'design', 'trace.md'), 'w', encoding='utf-8', newline='\n').write('\n'.join(out) + '\n')
